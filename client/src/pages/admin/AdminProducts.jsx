...
const AdminProducts = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      {/* Action Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-surface p-5 rounded-2xl border border-border shadow-sm">
        <div className="space-y-1">
          <h3 className="font-display font-bold text-base text-text-secondary">Merchandise Catalog</h3>
          <p className="font-sans text-xs text-text-secondary">Total products registered: <span className="font-bold text-brand-maroon-700">{products.length}</span></p>
        </div>
        <button
      ...
      {/* Filter Row */}
      <div className="bg-surface p-4 rounded-2xl border border-border shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        {/* Search */}
        ...
        <div className="flex items-center gap-2.5 w-full md:w-auto overflow-x-auto scrollbar-thin py-1">
          <div className="flex items-center gap-1.5 text-xs text-text-secondary font-semibold mr-1.5 flex-shrink-0">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl border text-[11px] font-bold capitalize transition-all duration-200 flex-shrink-0 ${
                categoryFilter === cat
                  ...
                  : 'bg-surface border-border text-text-secondary hover:border-brand-maroon-350'
              }`}
            >
      ...
      <div className="bg-surface rounded-2xl border border-border shadow-premium overflow-hidden">
        {productsLoading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <span className="text-xs font-sans text-text-secondary font-semibold">Updating catalog list...</span>
          </div>
        ) : products.length === 0 ? (
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-4.5">
                          <div className="w-11 h-11 rounded-lg border border-border overflow-hidden flex-shrink-0 bg-brand-dark-50">
                            <img 
                          ...
                          <div className="min-w-0">
                            <h4 className="font-display font-bold text-sm text-text-secondary truncate max-w-[240px] leading-snug">
                              {prod.name}
                            </h4>
                            <div className="flex items-center gap-1.5 mt-1">
                              {!prod.isActive && (
                                <span className="inline-flex px-1.5 py-0.5 rounded border border-border bg-brand-dark-100 text-text-secondary text-[9px] font-bold uppercase tracking-wider">
                                  Inactive
                                </span>
                              )}
                              {prod.isFeatured && (
                      ...
                      {/* Category */}
                      <td className="py-4 px-6 text-text-secondary font-semibold capitalize">
                      ...
                      <td className="py-4 px-6 font-bold text-text-secondary">
                        ...
                        {prod.comparePrice && (
                          <span className="text-[10px] text-text-secondary font-normal line-through ml-1.5">
                            ₹{prod.comparePrice}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-4 px-6 text-right space-x-1">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-text-secondary hover:bg-brand-dark-100 hover:text-text-secondary transition-colors"
                          title="Edit Product"
                        >
                        ...
                        <button
                          ...
                          className={`p-1.5 rounded-lg transition-colors ${
                            prod.isActive
                              ? 'text-text-secondary hover:bg-brand-dark-100 hover:text-text-secondary'
