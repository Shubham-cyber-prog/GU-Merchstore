...
const AdminAnalytics = () => {
        ...
        {topProducts.slice(0, 5).map((prod, idx) => {
          ...
          return (
            <div key={prod.productId || idx} className="space-y-2 font-sans">
              ...
              <div className="w-full h-2.5 bg-surface/5 rounded-full overflow-hidden">
                <div
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: Users,
      iconBg: 'bg-surface/5 border-white/10 text-white/60',
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-surface/5 hover:bg-surface/10 border border-white/10 text-white/70 hover:text-white text-xs font-semibold transition-all disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-[#12081a]/80 border border-white/5 p-6 rounded-2xl lg:col-span-2 space-y-5">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <div className="flex items-center gap-1 bg-surface/5 p-1 rounded-xl border border-white/10">
      ...
      {usersModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-fadeIn">
          ...
          <div className="relative bg-[#12081a] border border-[#d4af37]/20 rounded-3xl p-6 sm:p-8 shadow-premium w-full max-w-4xl max-h-[85vh] z-10 flex flex-col animate-slideUp text-left">
            <button 
              onClick={() => setUsersModalOpen(false)}
              className="absolute top-6 right-6 p-2 text-white/40 hover:text-white hover:bg-surface/5 rounded-xl transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="relative mb-5">
              ...
              <input
                type="text"
                className="w-full pl-10 pr-4 py-3 bg-surface/5 border border-white/10 rounded-xl text-sm text-white placeholder-white/20 outline-none focus:border-[#d4af37]/60 focus:ring-1 focus:ring-[#d4af37]/60 transition-all font-sans"
                      ...
                      <div 
                        ...
                        className={`border rounded-2xl overflow-hidden transition-all duration-200 ${
                          isExpanded 
                            ...
                            : 'border-white/5 bg-surface/[0.02] hover:border-white/10'
                        }`}
                      >
                                ...
                                {!u.addresses || u.addresses.length === 0 ? (
                                  ...
                                  <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1">
                                    {u.addresses.map((addr, idx) => (
                                      <div key={idx} className="p-2.5 rounded-lg bg-surface/5 border border-white/5 leading-relaxed text-white/70">
                                        <p className="font-semibold text-[#d4af37] text-[11px]">{addr.fullName} {addr.isDefault && <span className="text-[9px] uppercase font-bold text-white/40 border border-white/10 px-1 rounded ml-1 bg-surface/5">Default</span>}</p>
                              ...
                              {orders.length === 0 ? (
                                ...
                                <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-1">
                                  {orders.map((ord) => {
                                    ...
                                    return (
                                      <div key={ord._id} className="p-3 bg-surface/5 border border-white/5 rounded-xl hover:border-white/10 transition-colors flex justify-between items-center gap-3">
