  ...
  if (!cartLoading && safeCartItems.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        <h3 className="font-display font-bold text-xl text-text-secondary">Your Cart is Empty</h3>
        <p className="font-sans text-text-secondary mt-2">Add items to your cart before checking out.</p>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 min-h-screen text-left">
        ...
        <div className="flex items-center w-full">
          ...
          <div className="flex flex-col items-center relative">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-display font-bold text-xs ${
              step >= 1 ? 'bg-primary text-white' : 'bg-brand-dark-200 text-text-secondary'
            }`}>
              {step > 1 ? <Check className="w-4.5 h-4.5" /> : '1'}
            </div>
            <span className="font-sans text-xxs font-bold text-text-secondary absolute -bottom-5 w-max">Shipping</span>
          </div>

          <div className={`flex-grow border-t-2 h-0.5 mx-2 ${step > 1 ? 'border-brand-maroon-700' : 'border-border'}`}></div>

          {/* Step 2 */}
          <div className="flex flex-col items-center relative">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-display font-bold text-xs ${
              step >= 2 ? 'bg-primary text-white' : 'bg-brand-dark-200 text-text-secondary'
            }`}>
              {step > 2 ? <Check className="w-4.5 h-4.5" /> : '2'}
            </div>
            <span className="font-sans text-xxs font-bold text-text-secondary absolute -bottom-5 w-max">Payment</span>
          </div>

          <div className={`flex-grow border-t-2 h-0.5 mx-2 ${step > 2 ? 'border-brand-maroon-700' : 'border-border'}`}></div>

          {/* Step 3 */}
          <div className="flex flex-col items-center relative">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-display font-bold text-xs ${
              step >= 3 ? 'bg-primary text-white' : 'bg-brand-dark-200 text-text-secondary'
            }`}>
              3
            </div>
            <span className="font-sans text-xxs font-bold text-text-secondary absolute -bottom-5 w-max">Review</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {step === 2 && (
            <div className="bg-surface border border-border rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <div className="text-left">
                    <h3 className="font-display font-bold text-sm text-text">Online Card (Stripe)</h3>
                    <p className="font-sans text-[11px] text-text-secondary mt-1">Instant secure authorization via credit/debit card.</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="text-left">
                    <h3 className="font-display font-bold text-sm text-text">UPI (QR Code)</h3>
                    <p className="font-sans text-[11px] text-text-secondary mt-1">Scan QR code using GPay, PhonePe, or Paytm.</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="text-left">
                    <h3 className="font-display font-bold text-sm text-text">Collection (COD)</h3>
                    <p className="font-sans text-[11px] text-text-secondary mt-1">Pay cash when collecting items at the counter.</p>
                  </div>
                </div>
              </div>
              ...
              {paymentMethod === 'stripe' && (
                <div className="border border-border rounded-2xl p-5 bg-brand-dark-50 space-y-4 max-w-md animate-fadeIn">
              ...
              {paymentMethod === 'upi' && (
                <div className="border border-border rounded-2xl p-6 bg-brand-dark-50 space-y-5 max-w-md animate-fadeIn">
                  <h4 className="font-sans font-bold text-xs uppercase tracking-wider text-text-secondary">
                    UPI Payment Details
                  </h4>
                  <div className="flex flex-col items-center gap-3 bg-surface p-4 rounded-xl border border-border">
                    ...
                    <img 
                      ...
                      alt="UPI QR Code" 
                      className="w-40 h-40 object-contain border border-border rounded-lg p-1 bg-surface"
                    />
                  ...
                  <div className="space-y-2">
                    <label className="block text-left text-xs font-bold text-text-secondary">
          ...
          {step === 3 && (
            <div className="bg-surface border border-border rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <div className="border border-border rounded-2xl divide-y divide-brand-dark-200 overflow-hidden bg-brand-dark-50/50">
                {/* Shipping summary */}
                <div className="p-4 flex justify-between gap-4 text-xs font-sans">
                  <div className="text-left space-y-1">
                    <span className="font-bold uppercase tracking-wider text-text-secondary">Shipping To</span>
                  ...
                  <button onClick={() => setStep(1)} className="font-bold text-primary hover:text-primary/80 h-fit">Edit</button>
                </div>

                {/* Payment summary */}
                <div className="p-4 flex justify-between gap-4 text-xs font-sans">
                  <div className="text-left space-y-1">
                    <span className="font-bold uppercase tracking-wider text-text-secondary">Payment Mode</span>
                    <p className="text-text-secondary font-semibold capitalize">
                      {paymentMethod === 'stripe' 
                        ? 'Online Card (Stripe)' 
                        : paymentMethod === 'upi'
                  ...
                  <button onClick={() => setStep(2)} className="font-bold text-primary hover:text-primary/80 h-fit">Edit</button>
                </div>
              </div>

              {/* Items checklist */}
              <div className="space-y-4">
                <h3 className="font-sans font-bold text-xs uppercase tracking-wider text-text-secondary">Items List</h3>
                ...
                {safeCartItems.map((item, idx) => (
                    <div key={idx} className="py-3 flex justify-between items-center text-sm font-sans">
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-text-secondary">{item.name || item.product?.name}</span>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {/* Pricing Totals */}
          <div className="bg-surface border border-border rounded-3xl p-6 shadow-premium space-y-5">
            <h3 className="font-display font-bold text-base text-text border-b border-border pb-3">
              Order Breakdown
            </h3>

            <div className="space-y-3.5 text-xs font-sans font-semibold text-text-secondary">
              <div className="flex justify-between">
                <span>Items Subtotal</span>
                <span className="text-text-secondary">₹{subtotal.toLocaleString('en-IN')}.00</span>
              </div>
              <div className="flex justify-between">
            ...
            <div className="flex justify-between items-baseline">
              <span className="font-display font-bold text-sm text-text">Total Payable</span>
