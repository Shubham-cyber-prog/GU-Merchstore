...
const ProductFormModal = ({ isOpen, onClose, onSave, product, isSaving }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-dark-900/60 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-surface rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl border border-border animate-scaleUp">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-border flex-shrink-0">
          <h2 className="font-display font-extrabold text-lg text-text-secondary">
            {product ? 'Edit University Product' : 'Add New Merchandise'}
          </h2>
          <button 
            onClick={onClose}
            className="p-1.5 rounded-lg text-text-secondary hover:bg-brand-dark-50 hover:text-text-secondary transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-bold font-sans uppercase tracking-wider text-text-secondary">Product Name</label>
              <input 
                type="text" 
            ...
            {/* Category */}
            <div className="space-y-1.5">
              <label className="block text-xs font-bold font-sans uppercase tracking-wider text-text-secondary">Category</label>
              <select
            ...
            <div className="col-span-1 md:col-span-2 space-y-1.5">
              <label className="block text-xs font-bold font-sans uppercase tracking-wider text-text-secondary">Description</label>
              <textarea 
                rows="3"
                className="input-field py-2.5 text-sm"
            ...
            {/* Prices */}
            <div className="space-y-1.5">
              <label className="block text-xs font-bold font-sans uppercase tracking-wider text-text-secondary">Selling Price (₹)</label>
              <input 
                type="number" 
                className="input-field py-2.5 text-sm"
                placeholder="e.g. 999"
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-bold font-sans uppercase tracking-wider text-text-secondary">Compare Price (₹) <span className="text-[10px] text-text-secondary font-normal font-sans lowercase">(shows strikethrough)</span></label>
              <input 
                type="number" 
                className="input-field py-2.5 text-sm"
            ...
            {/* Tags */}
            <div className="space-y-1.5">
              <label className="block text-xs font-bold font-sans uppercase tracking-wider text-text-secondary">Tags <span className="text-[10px] text-text-secondary font-normal font-sans lowercase">(comma separated)</span></label>
              <input 
                type="text" 
                className="input-field py-2.5 text-sm"
            ...
            <div className="flex gap-6 items-center pt-5">
              <label className="flex items-center gap-2 cursor-pointer">
                <input 
                  type="checkbox"
                  className="w-4 h-4 accent-brand-maroon-700 rounded border-border focus:ring-brand-maroon-500"
                  checked={isFeatured}
                  onChange={e => setIsFeatured(e.target.checked)}
                />
                <span className="text-xs font-bold font-sans uppercase tracking-wider text-text-secondary">Featured Item</span>
              </label>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-xs font-bold font-sans uppercase tracking-wider text-text-secondary">In Stock / Active</span>
              </label>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <div className="space-y-0.5">
                <h3 className="font-display font-bold text-sm text-text-secondary">Sizing & Inventory Mapping</h3>
                <p className="font-sans text-[11px] text-text-secondary">Add sizes, configure SKU identifiers and stock levels.</p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-2.5">
              {sizes.map((sz, idx) => (
                <div key={idx} className="flex gap-4 items-center bg-brand-dark-50 p-3 rounded-xl border border-border animate-fadeIn">
                  {/* Size Selector */}
                  <div className="w-1/4">
                    <select
          ...
          <div className="space-y-4">
            <div className="space-y-0.5">
              <h3 className="font-display font-bold text-sm text-text-secondary">Product Images (Max 5)</h3>
              <p className="font-sans text-[11px] text-text-secondary">Upload merchandise pictures (JPG/PNG). Multi-image uploads are supported.</p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
              ...
              {existingImages.map((img, idx) => {
                ...
                return (
                  <div key={`exist-${idx}`} className={`relative aspect-square border rounded-xl overflow-hidden group ${
                    isRemoved ? 'border-red-300 opacity-60 bg-red-50' : 'border-border'
                  }`}>
                    <img src={img} alt="product" className="w-full h-full object-cover" />
                    {isRemoved ? (
                      <div className="absolute inset-0 bg-red-900/10 flex flex-col items-center justify-center p-2">
                        <span className="text-[9px] font-bold text-red-700 uppercase bg-surface border border-red-200 px-1 py-0.5 rounded shadow">Marked delete</span>
                        <button
                          type="button"
                          onClick={() => handleUndoRemoveExistingImage(img)}
                          className="mt-1.5 text-[10px] font-bold text-brand-maroon-700 hover:underline bg-surface px-2 py-0.5 rounded shadow"
                        >
                          Undo
                        </button>
                      </div>
              ...
              {activeExistingImages.length + newImages.length < 5 && (
                <label className="border-2 border-dashed border-border hover:border-brand-maroon-500 rounded-xl flex flex-col items-center justify-center aspect-square cursor-pointer hover:bg-brand-dark-50/50 transition-all duration-300">
                  <Upload className="w-5 h-5 text-text-secondary mb-1" />
                  <span className="text-[10px] font-sans font-bold text-text-secondary uppercase tracking-wider">Upload Image</span>
                  <input 
                    type="file" 
                    multiple 
                    accept="image/*" 
        ...
        {/* Footer actions */}
        <div className="p-5 border-t border-border flex justify-end gap-3 flex-shrink-0 bg-brand-dark-50 rounded-b-2xl">
          <button
            type="button"
            onClick={onClose}
